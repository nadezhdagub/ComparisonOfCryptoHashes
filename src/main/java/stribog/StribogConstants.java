...
public class StribogConstants {
  ...
  /**
   * Таблиц замен
   * http://protect.gost.ru/v.aspx?control=8&baseC=-1&page=0&month=-1&year=-1&search=&RegNum=1&DocOnPageCount=15&id=224241&pageK=8A5E965D-A695-4C6C-9DC0-76EFE1C804D5
   */
  ...
  public int[] numByte(int num) {
    ...
    result[62] = (num * 8) >> 8;                          //исходя из того, что любое число, теоретически подаваемое в функцию, можно закодировать 2 байтам
    result[63] = (num * 8) & 0xFF;                        //поскольку в Java тип byte хранит значения от -128 до 127(первый бит отводится под знак), мы вынуждены интерпретировать значения в положительный диапазон https://habr.com/ru/post/225901/
    return result;
  }
